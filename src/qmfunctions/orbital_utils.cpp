...
/** @brief Returns the size of the coefficients of all nodes in the vector */
int orbital::get_size_nodes(const QMFunctionVector &Phi, IntVector &sNodes) {
    int nOrbs = Phi.size();
    int totsize = 0;
    for (int i = 0; i < nOrbs; i++){
        if( Phi[i].hasReal()){
            int fac = Phi[i].real().getKp1_d()*8;//number of coeff in one node
            fac *= sizeof(double);// Number of Bytes in one node
            sNodes[i] = (int)1.0*(fac * Phi[i].getNNodes(NUMBER::Total)/1024); //kBytes in one orbital
            totsize += sNodes[i];
        }
    }
    return totsize;
...
/** @brief Prints statistics about the size of orbitals */
int orbital::print_size_nodes(const OrbitalVector &Phi, char* txt, bool all) {
    ...
    double nMax = 0, vMax = 0; //node max, vector max
...
/** @brief Prints statistics about the size of orbitals */
int orbital::print_size_nodes(const QMFunctionVector &Phi, char* txt) {
    int nOrbs = Phi.size();
    IntVector sNodes = IntVector::Zero(nOrbs);
    int sVec = get_size_nodes(Phi, sNodes);
    int nMax = 0, vMax = 0; //node max, vector max
    int nMin = 999999999, vMin = 999999999;
    int nSum = 0, vSum = 0;
    int nOwnOrbs = 0, OwnSumMax = 0, OwnSumMin = 999999999;
    if(mpi::orb_rank==0)std::cout<<"OrbitalVector sizes statistics (MB) "<<txt<<std::endl;
    //stats for own orbitals
    for (int i = 0; i < nOrbs; i++){
        if(sNodes[i] > 0){
    ...
    IntMatrix VecStats = IntMatrix::Zero(4, mpi::orb_size);
    VecStats(0,mpi::orb_rank) = nMax;
    VecStats(1,mpi::orb_rank) = nMin;
    VecStats(2,mpi::orb_rank) = nSum;
    VecStats(3,mpi::orb_rank) = nOwnOrbs;
    ...
    if(mpi::orb_rank==0)std::cout<<"Total orbvec "<<(int)1.0*vSum/(1024*1024)<<", ";
    if(mpi::orb_rank==0)std::cout<<"Av/MPI "<<(int)1.0*vSum/(1024*1024)/mpi::orb_size<<", ";
    if(mpi::orb_rank==0)std::cout<<"Max/MPI "<<(int)1.0*OwnSumMax/(1024*1024)<<", ";
    if(mpi::orb_rank==0)std::cout<<"Max/orb "<<(int)1.0*vMax/(1024*1024)<<", ";
    if(mpi::orb_rank==0)std::cout<<"Min/orb "<<(int)1.0*vMin/(1024*1024)<<", ";
    if(mpi::orb_rank==0)std::cout<<"Total master "<<(int)(Printer::printMem("",true)*4.0/(1024.0))<<" MB"<<std::endl;
